$(document).ready(function(){
  ...
  // Variables para enviar al controlador
  var titulo;
  var descripcion;
  var lock;
  ...
  //aplicar un span a los h3 que estan dentro de los objetos para lograr el efecto mecanografico
  $span = $("<span>&nbsp;</span>");
  $(".widget-title-header h3").append($span);

 //establecer backgrund del color de la cabecera donde se encuentra el span para simular efecto mecanografico
  $(".widget-title-header h3").children("span").each(function(){
 ...
 $("#thems a").click(function(){
	 ...
	 /*
	 	...
	 	hice por cada seccion y de esta manera solo lo creo una vez y lo aplico a todos las secciones
																									*/
	 ...
	 $btnNext = $("<i  class='btn fa fa-share bg-blue' title='Cambiar Descripción'/>");
	 $btnSave = $("<i  class='btn fa fa-check bg-green ' title='Guardar Cambios'/>");
	 $btnCancel = $("<i  class='btn fa fa-undo bg-red' title='Cancelar Cambios'/>");
	 $btnPrev = $("<i  class='btn fa fa-reply bg-blue' title='Cambiar Título/Bloqueo'/>");
	 ...
	 $h4Title = $("<h4/>").text("Ingresa el nuevo título");
...
//transicion
 ...
 $(".fa-refresh").click(function(){
	 ...
	 $cont.parent().parent().css("height",""+$(".box").css("height")+"");//aplicar misma medidad a todas las cajas de los objetos
...
//click en el icono regresar para cancelar la actualizacion del objeto
...
$(".box").on("click",".btn.fa-reply",function(){
	...
	$(this).parent().parent().prev().children().first().text("Ingrese el nuevo titulo");
	$(this).parent().parent().prev().show();

});

$("div.description-block>.fa-remove").click(function(e){
  alertify.confirm("¿Esta seguro que desea eliminar?", function() {
    if (e) {
      // user clicked "ok",
    }
    else {
