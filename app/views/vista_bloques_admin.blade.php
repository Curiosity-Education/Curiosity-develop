...
@section('migas')
  ...
  <li><a href="/adminInteligencia{{$provieneNivel[0]['id']}}">{{$provieneNivel[0]['nombre']}}</a></li>
  <li class="fa fa-angle-right separatorBrand"></li>
  <li><a href="/adminBloque{{$perteneciente}}" class="brandActive">{{$objetos2[0]['nombre']}}</a></li>
@stop
  ...
  <div class='col-md-4 objeto' data-id = {{ $objeto->id }} data-id-remove = {{$objeto->id}}>
    <div class='box box-widget widget-title'>
      <div class="widget-title-header {{ $objeto->bg_color }}">
        <h3 class='widget-title-set text-center' data-descrip='{{$objeto->descripcion}}' data-estatus='{{$objeto->estatus}}' id={{$objeto->id}}>{{$objeto->nombre}}</h3>
        <h5 class='widget-title-desc'></h5>
      </div>
      <div class='widget-title-image'>
        <img class='img-circle img-effect tooltipShow' title="Cambiar imagen" src='/packages/images/bloques/{{$objeto->imagen}}' data-id-img={{$objeto->id}}>
      </div>
        ...
        @if(!Entrust::can('subir_juegos') || Auth::user()->hasRole('root'))
          ...
          <div class='col-xs-4'>
            <div class='description-block btnIn'
              data-id-bloque={{$objeto->id}}
              data-id-inteligencia={{$perteneciente}}
              data-id-nivel={{$provieneNivel[0]['id']}}>
...
<!-- Sección de Administración -->
  <div class="col-md-12" id="adminSection" hidden="hidden">
    ...
    <div class="form-group text-right">
      ...
      <button type="button" class="btn btn-primary" data-inteligencia={{$perteneciente}} data-nivelid={{$provieneNivel[0]['id']}} id="enviarEnv">
...
@section('mi_js')
  {{HTML::script('/packages/js/curiosity/bloquesAdmin.js')}}
@stop
